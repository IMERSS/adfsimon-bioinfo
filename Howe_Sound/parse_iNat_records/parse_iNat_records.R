...
iNat.obs <- read.csv("iNat_records/Howe_Sound_iNat_Ascomycota_2023-10-18-obs.csv")
...
# Otherwise filter obs from trusted observers
...
Lichenized.Ascomycota.obs<- subset(Lichenized.Ascomycota.obs, Order != 'Venturiales' & Order != 'Sareales' & Family != 'Stictidaceae' & Order != 'Eurotiales' & Order != 'Chaetothyriales' & Order != 'Capnodiales' & Order != 'Botryosphaeriales')
