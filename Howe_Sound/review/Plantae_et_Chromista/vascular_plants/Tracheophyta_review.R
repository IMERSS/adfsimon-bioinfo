# Script to compare biodiversity observations against a historical baseline
...
baseline.names.col.no <- 2
baseline.names.row.no <- nrow(baseline)

baseline.names <- data.frame(matrix(ncol = baseline.names.col.no, nrow = baseline.names.row.no))
names(baseline.names) <- c("scientificName","records")

baseline.names$scientificName <- baseline$scientificName
baseline.names$records <- "yes"

summary.names.col.no <- 2
summary.names.row.no <- nrow(summary)

summary.names <- data.frame(matrix(ncol = summary.names.col.no, nrow = summary.names.row.no))
names(summary.names) <- c("scientificName","records")

summary.names$scientificName <- summary$scientificName
summary.names$records <- "yes"

missing.taxa <- anti_join(baseline.names, summary.names)
missing.taxa$records <- "no"
