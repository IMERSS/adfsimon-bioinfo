...
Algae.baseline <- read.csv("summary/Charophyta_Chlorophyta_Rhodophyta_Chromista_baseline_2022.csv")

# Read iNaturalist obs summary

iNat.obs.summary <- read.csv("../../../1_split/macroalgae_zooplankton_and_phytoplankton/outputs/Algae_iNat_summary.csv")
...
matched.iNat.obs.summary <- inner_join(Algae.baseline, iNat.obs.summary, by = 'Taxon')
...
matched.iNat.date.observed <- inner_join(Algae.baseline, matched.iNat.obs.summary, by = c('Taxon','First.Observed'))

# Matched summary

Algae.summary.matched <- inner_join(Algae.baseline, matched.iNat.date.observed, by = 'Taxon')
names(Algae.summary.matched)
Algae.summary.matched <- Algae.summary.matched[,c(1:33)]
colnames(Algae.summary.matched) <- colnames(Algae.baseline)
other.summary.column.names <- c('taxon_species_name', 'taxon_hybrid_name', 'taxon_subspecies_name','taxon_variety_name')
other.summary.columns <- data.frame(matrix(ncol=length(other.summary.column.names),nrow=nrow(Algae.summary.matched)))
names(other.summary.columns) <- other.summary.column.names
Algae.summary.matched <- cbind(Algae.summary.matched,other.summary.columns)
names(Algae.summary.matched)
names(Algae.summary.matched) <- c('Taxon', 'Taxon Author', 'Subtaxon Author', 'Common Name', 'Kingdom', 'Phylum', 'Subphylum', 'Superclass', 'Class', 'Subclass', 'Superorder', 'Order', 'Suborder', 'Superfamily', 'Family', 'Subfamily', 'Tribe', 'Genus', 'Origin', 'Status', 'Reporting Status', 'Observation', 'Collected/Reported (y-m-d)', 'Collector/Source', 'Collection/List', 'Accession Number', 'GBIF ID', 'First Observed' , 'Observer', 'iNaturalist Link', 'Notes', 'ID', 'Stats Code', 'Species', 'Hybrid', 'Subspecies','Variety')
Algae.summary.matched <- Algae.summary.matched %>% select('Taxon', 'Taxon Author', 'Subtaxon Author', 'Common Name', 'Kingdom', 'Phylum', 'Subphylum', 'Superclass', 'Class', 'Subclass', 'Superorder', 'Order', 'Suborder', 'Superfamily', 'Family', 'Subfamily', 'Tribe', 'Genus', 'Species', 'Hybrid', 'Subspecies', 'Variety', 'Origin', 'Status', 'Reporting Status', 'Observation', 'Collected/Reported (y-m-d)', 'Collector/Source', 'Collection/List', 'Accession Number', 'GBIF ID', 'First Observed', 'iNaturalist Link', 'Notes', 'ID', 'Stats Code')
...
Algae.summary.matched$Species <- matched.iNat.date.observed$taxon_species_name[match(unlist(Algae.summary.matched$Taxon), matched.iNat.date.observed$Taxon)]

# Unmatched summary

unmatched.iNat.obs = anti_join(iNat.obs.summary, Algae.summary.matched, by = c("Taxon" = "Taxon"))
...
# names(unmatched.iNat.obs.summary)  # Do this manually later...
...
other.summary.columns <- data.frame(matrix(ncol=length(other.summary.column.names),nrow=nrow(Algae.baseline)))
names(other.summary.columns) <- other.summary.column.names
Algae.review.summary <- cbind(Algae.baseline,other.summary.columns)
names(Algae.review.summary)
Algae.review.summary <- Algae.review.summary %>% select(Taxon, Taxon.Author, Subtaxon.Author, Common.Name, Kingdom, Phylum, Subphylum, Superclass, Class, Subclass, Superorder, Order, Suborder, Superfamily, Family, Subfamily, Tribe, Genus, Species, Hybrid, Subspecies, Variety, Origin, Status, Reporting.Status, Observation, Collected.Reported..y.m.d., Collector.Source, Collection.List, Accession.Number, GBIF.ID, First.Observed, Observer, iNaturalist.Link, Notes, ID, Stats.Code)
names(Algae.review.summary) <- c('Taxon', 'Taxon Author', 'Subtaxon Author', 'Common Name', 'Kingdom', 'Phylum', 'Subphylum', 'Superclass', 'Class', 'Subclass', 'Superorder', 'Order', 'Suborder', 'Superfamily', 'Family', 'Subfamily', 'Tribe', 'Genus', 'Species', 'Hybrid', 'Subspecies', 'Variety', 'Origin', 'Status', 'Reporting Status', 'Observation', 'Collected/Reported (y-m-d)', 'Collector/Source', 'Collection/List', 'Accession Number', 'GBIF ID', 'First Observed', 'Observer', 'iNaturalist Link', 'Notes', 'ID', 'Stats Code')
...
Algae.review.summary$Species <- Algae.summary.matched$Species[match(unlist(Algae.review.summary$Taxon), Algae.summary.matched$Taxon)]

# Merge baseline and unmatched summary for review 

Algae.review.summary <- rbind(Algae.review.summary, unmatched.iNat.obs.summary)

# Add column for National Status Ranking (and rename 'Status' to 'Provincial Status')

Algae.review.summary$National_Status <- NA
...
Algae.review.summary[is.na(Algae.review.summary)] <- ""

write.csv(Algae.review.summary, "outputs/Charophyta_Chlorophyta_Rhodophyta_Chromista_review_summary.csv")