...
baseline <- read.csv("summaries/Charophyta_Chlorophyta_Rhodophyta_Chromista_review_summary_reviewed_2023-04-18.csv")

# Read catalog of consolidated occurrence records

records <- read.csv("../../../consolidate_records/Plantae_et_Chromista/macroalgae_zooplankton_and_phytoplankton/synthesized/...")
...
matched.iNat.obs.summary <- inner_join(baseline, iNat.obs.summary, by = c("Genus", "Hybrid", "Species", "Subspecies", "Variety"))
...
matched.iNat.date.observed <- inner_join(baseline, matched.iNat.obs.summary, by = c('Taxon','First.Observed'))

# Matched summary

Algae.summary.matched <- inner_join(baseline, matched.iNat.date.observed, by = 'Taxon')
names(Algae.summary.matched)
Algae.summary.matched <- Algae.summary.matched[,c(1:38)]
colnames(Algae.summary.matched) <- colnames(baseline)
...
names(baseline) <- c('Taxon', 'Taxon Author', 'Subtaxon Author', 'Common Name', 'Kingdom', 'Phylum', 'Subphylum', 'Superclass', 'Class', 'Subclass', 'Superorder', 'Order', 'Suborder', 'Superfamily', 'Family', 'Subfamily', 'Tribe', 'Genus', 'Species', 'Hybrid', 'Subspecies', 'Variety', 'Origin', 'Provincial Status', 'National Status','Reporting Status', 'Observation', 'Collected/Reported (y-m-d)', 'Collector/Source', 'Collection/List', 'Accession Number', 'GBIF ID', 'First Observed', 'Observer', 'iNaturalist Link', 'Notes', 'ID', 'Stats Code')
...
Algae.review.summary <- rbind(baseline, unmatched.iNat.obs.summary)
