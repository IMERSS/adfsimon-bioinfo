...
Algae.baseline <- read.csv("summaries/Charophyta_Chlorophyta_Rhodophyta_Chromista_review_summary_reviewed_2022-10-30.csv")
...
Algae.baseline[is.na(Algae.baseline)] <- ""

# Match observation summary against baseline summary by Genus, Species, Subspecies, Variety

matched.iNat.obs.summary <- inner_join(Algae.baseline, iNat.obs.summary, by = c("Genus", "Hybrid", "Species", "Subspecies", "Variety"))
...
Algae.summary.matched <- Algae.summary.matched[,c(1:38)]
...
unmatched.iNat.obs = anti_join(iNat.obs.summary, Algae.summary.matched, by = c("Genus", "Hybrid", "Species", "Subspecies", "Variety"))
...
names(Algae.baseline) <- c('Taxon', 'Taxon Author', 'Subtaxon Author', 'Common Name', 'Kingdom', 'Phylum', 'Subphylum', 'Superclass', 'Class', 'Subclass', 'Superorder', 'Order', 'Suborder', 'Superfamily', 'Family', 'Subfamily', 'Tribe', 'Genus', 'Species', 'Hybrid', 'Subspecies', 'Variety', 'Origin', 'Provincial Status', 'National Status','Reporting Status', 'Observation', 'Collected/Reported (y-m-d)', 'Collector/Source', 'Collection/List', 'Accession Number', 'GBIF ID', 'First Observed', 'Observer', 'iNaturalist Link', 'Notes', 'ID', 'Stats Code')
...
Algae.review.summary <- rbind(Algae.baseline, unmatched.iNat.obs.summary)
