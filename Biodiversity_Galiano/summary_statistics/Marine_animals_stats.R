#### R Script for visualizing marine animal reporting
...
## ------------------------------------------------------------
## CUMULATIVE KNOWLEDGE TABLE BY PHYLUM
## ------------------------------------------------------------

taxon_phylum_status <- df %>%
  filter(!is.na(Phylum), !is.na(Taxon), Taxon != "") %>%
...
phylum_diversity_summary <- taxon_phylum_status %>%
  count(Phylum, knowledge_category, name = "n_taxa") %>%
  group_by(Phylum) %>%
  mutate(total_taxa = sum(n_taxa)) %>%
  ungroup()

# Order phyla by total diversity
phylum_order <- phylum_diversity_summary %>%
  distinct(Phylum, total_taxa) %>%
  arrange(desc(total_taxa), Phylum) %>%
  pull(Phylum)

phylum_diversity_summary$Phylum <- factor(
  phylum_diversity_summary$Phylum,
  levels = phylum_order
)
...
phylum_diversity_summary <- phylum_diversity_summary %>%
  mutate(
    Phylum_label = recode(as.character(Phylum), !!!phylum_label_map, .default = as.character(Phylum))
  )

# Preserve plotting order after relabeling
phylum_label_levels <- phylum_diversity_summary %>%
  distinct(Phylum, Phylum_label) %>%
  mutate(Phylum = factor(Phylum, levels = phylum_order)) %>%
  arrange(Phylum) %>%
  pull(Phylum_label)

phylum_diversity_summary$Phylum_label <- factor(
  phylum_diversity_summary$Phylum_label,
  levels = phylum_label_levels
)
...
## ------------------------------------------------------------
## BARPLOT: RAW COUNTS OF TAXA BY PHYLUM
## ------------------------------------------------------------
...
## ------------------------------------------------------------
## RANDOM EXAMPLE OBSERVATIONS TABLE BY PHYLUM × KNOWLEDGE CATEGORY
...
col_phylum         <- first_existing_col(df, c("Phylum"))
...
required_cols <- c(col_phylum, col_taxon, col_status)
if (any(is.na(required_cols))) {
  stop(
    "Missing one or more required columns for example-table generation: Phylum, Taxon, and/or Reporting.Status.",
    call. = FALSE
  )
}
...
example_pool <- df %>%
  mutate(
    Phylum_std = pull_or_na(., col_phylum),
  ...
  mutate(
    Phylum_std = na_if(str_trim(Phylum_std), ""),
...
## ------------------------------------------------------------
## ATTACH FRIENDLY PHYLUM LABELS IF AVAILABLE
## ------------------------------------------------------------

example_pool <- example_pool %>%
  mutate(
    Phylum_label = recode(Phylum_std, !!!phylum_label_map, .default = Phylum_std)
  )

## ------------------------------------------------------------
## SAMPLE ONE RANDOM EXAMPLE PER PHYLUM × KNOWLEDGE CATEGORY
## ------------------------------------------------------------
...
cat("\nRandom example observations by phylum × knowledge category:\n")
cat(
...
write.csv(
  example_table,
  "outputs/random_example_observations_by_phylum_recent_new_records.csv",
  row.names = FALSE
)

cat("\nSaved file: outputs/random_example_observations_by_phylum_recent_new_records.csv\n")


## ------------------------------------------------------------
## RANDOM EXAMPLE RECORDS BY PHYLUM + STORY TEXT
## ------------------------------------------------------------
...
# Helper: make a readable group phrase from phylum label
choose_group_phrase <- function(phylum_label, phylum_latin = NA_character_) {
  ...
  if (!is.na(phylum_latin) && nzchar(phylum_latin)) {
    return(phylum_latin)
  }
  "animals"
}
...
required_cols <- c(col_phylum, col_taxon, col_status_raw)
if (any(is.na(required_cols))) {
  stop(
    "Missing one or more required columns: Phylum, Taxon, and/or Reporting.Status.",
    call. = FALSE
  )
}
...
example_pool <- df %>%
  ...
  mutate(
    Phylum_std = na_if(stringr::str_trim(Phylum_std), ""),
  ...
  mutate(
    Phylum_label = dplyr::recode(Phylum_std, !!!phylum_label_map, .default = Phylum_std)
  )

## ------------------------------------------------------------
## SAMPLE ONE RANDOM EXAMPLE PER PHYLUM × CATEGORY
## ------------------------------------------------------------

marine_animal_story_examples <- example_pool %>%
  group_by(Phylum_std, Phylum_label, Knowledge_category) %>%
  slice_sample(n = 1) %>%
  ungroup() %>%
  mutate(
...
print(marine_animal_story_examples, n = nrow(marine_animal_story_examples))

write.csv(
  marine_animal_story_examples,
  "outputs/Marine_animal_reports_by_phylum.csv",
  row.names = FALSE
)

cat("\nSaved file: outputs/Marine_animal_reports_by_phylum.csv\n")


## ------------------------------------------------------------
## MARINE ANIMAL DIVERSITY SUMMARY BY PHYLUM
...
OUTPUT_TABLE <- TRUE
OUTPUT_DIR   <- "outputs"
OUTPUT_FILE  <- file.path(OUTPUT_DIR, "Marine_animal_diversity_summary_by_phylum.csv")
...
phylum_group_phrase <- function(x) {
...
required_cols <- c(col_taxon, col_phylum, col_status_raw)
if (any(is.na(required_cols))) {
  stop(
    "Missing one or more required columns: Taxon, Phylum, and/or Reporting.Status.",
    call. = FALSE
  )
}
...
dat <- df %>%
  ...
  filter(!is.na(Taxon_std), !is.na(Phylum_std), !is.na(status_std))
...
taxon_level <- dat %>%
  ...
  arrange(Phylum_label, Taxon_std, desc(status_rank), desc(contribution_year)) %>%
...
phylum_summary <- taxon_level %>%
  ...
  arrange(desc(total_taxa), Phylum_label)
...
## -----------------------------
top_confirmed <- phylum_summary %>%
  filter(confirmed_taxa > 0) %>%
  arrange(desc(confirmed_taxa), desc(recent_confirmed), Phylum_label) %>%
  slice_head(n = TOP_N_GAINERS)

top_new <- phylum_summary %>%
  filter(new_taxa > 0) %>%
  arrange(desc(new_taxa), desc(recent_new), Phylum_label) %>%
  slice_head(n = TOP_N_GAINERS)

top_recent <- phylum_summary %>%
  filter(recent_total > 0) %>%
  arrange(desc(recent_total), desc(recent_new), desc(recent_confirmed), Phylum_label) %>%
  slice_head(n = TOP_N_GAINERS)

top_attention <- phylum_summary %>%
  filter(remaining_historical > 0) %>%
  arrange(desc(remaining_historical), desc(unresolved_share), desc(total_taxa), Phylum_label) %>%
...
if (OUTPUT_TABLE) {
  ...
  phylum_summary_out <- phylum_summary %>%
    mutate(
  ...
  write_csv(phylum_summary_out, OUTPUT_FILE)
}
...
cat("\n")
cat("============================================================\n")
cat("MARINE ANIMAL DIVERSITY: BROAD-BRUSH SUMMARY BY PHYLUM\n")
cat("============================================================\n\n")
...
if (nrow(top_confirmed) > 0) {
  ...
  cat(
    paste(
      paste0(top_confirmed$Phylum_label, " (", top_confirmed$confirmed_taxa, ")"),
      collapse = ", "
    )
  )
  cat(".\n")
  if (any(top_confirmed$recent_confirmed > 0)) {
    cat(
      paste0(
        "Within the recent window, the leading phyla for historical confirmation were ",
        collapse_and(
          paste0(
            top_confirmed$Phylum_label[top_confirmed$recent_confirmed > 0],
...
if (nrow(top_new) > 0) {
  ...
  cat(
    paste(
      paste0(top_new$Phylum_label, " (", top_new$new_taxa, ")"),
      collapse = ", "
    )
  )
  cat(".\n")
  if (any(top_new$recent_new > 0)) {
    cat(
      paste0(
        ...
        collapse_and(
          paste0(
            top_new$Phylum_label[top_new$recent_new > 0],
...
if (nrow(top_recent) > 0) {
  cat(
    paste0(
      "Looking specifically at recent activity, the phyla showing the most momentum in ",
      RECENT_YEAR_MIN, "–", RECENT_YEAR_MAX, " were ",
      collapse_and(paste0(top_recent$Phylum_label, " (", top_recent$recent_total, ")")),
      ". These groups account for much of the recent growth in contemporary knowledge of the island biota.\n\n"
    )
  )
}
...
if (nrow(top_attention) > 0) {
  cat(
    paste0(
      "The phyla still requiring the most attention for confirmation of historical reports are ",
      collapse_and(
        paste0(
  ...
  cat(
    paste0(
      "These phyla stand out either because they contain the largest absolute backlog of historical reports, ",
...
cat(
  paste0(
    ...
    " indicates that contemporary community observations are continuing to make measurable gains, while also helping reveal which phyla still contain the largest historical knowledge gaps.\n\n"
  )
)

cat("------------------------------------------------------------\n")
cat("PHYLUM-LEVEL SNAPSHOT\n")
cat("------------------------------------------------------------\n\n")
