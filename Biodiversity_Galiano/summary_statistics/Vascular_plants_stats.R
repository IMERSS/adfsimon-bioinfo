...
MAX_CONTEMPORARY_CONTRIBUTORS <- 86
...
PLOT_TITLE <- "Biodiversity Galiano 2026: vascular plants"
...
VASCULAR_PLANTS <- read.csv(
  "../review/Plantae_et_Chromista/vascular_plants/summaries/Tracheophyta_summary_2026-04-04.csv",
...
names(VASCULAR_PLANTS) <- updated_names
...
## ------------------------------------------------------------
## VASCULAR PLANT ORDER SUMMARY FOR CUMULATIVE KNOWLEDGE BARPLOT
## ------------------------------------------------------------
...
## ============================================================
## VASCULAR PLANT DIVERSITY: CUMULATIVE KNOWLEDGE BY ORDER
## Fully regenerated order-based script
## ============================================================
...
## ------------------------------------------------------------
## VASCULAR PLANT ORDER FRIENDLY LABELS
## ------------------------------------------------------------

order_label_map <- c(
  "Alismatales"     = "Water-plantains & allies (Alismatales)",
  "Apiales"         = "Carrot & ginseng (Apiales)",
  "Aquifoliales"    = "Holly (Aquifoliales)",
  "Asparagales"     = "Asparagus & orchids (Asparagales)",
  "Asterales"       = "Sunflowers (Asterales)",
  "Boraginales"     = "Borage (Boraginales)",
  "Brassicales"     = "Mustard (Brassicales)",
  "Caryophyllales"  = "Pink & cactus (Caryophyllales)",
  "Celastrales"     = "Bittersweet (Celastrales)",
  "Ceratophyllales" = "Coontails (Ceratophyllales)",
  "Cornales"        = "Dogwoods (Cornales)",
  "Dipsacales"      = "Honeysuckles (Dipsacales)",
  "Equisetales"     = "Horsetails (Equisetales)",
  "Ericales"        = "Heath & blueberries (Ericales)",
  "Fabales"         = "Legumes (Fabales)",
  "Fagales"         = "Beech & oak (Fagales)",
  "Gentianales"     = "Gentians (Gentianales)",
  "Geraniales"      = "Geraniums (Geraniales)",
  "Lamiales"        = "Mints (Lamiales)",
  "Liliales"        = "Lilies (Liliales)",
  "Lycopodiales"    = "Clubmoss (Lycopodiales)",
  "Malpighiales"    = "Spurge & violets (Malpighiales)",
  "Malvales"        = "Mallows (Malvales)",
  "Myrtales"        = "Myrtles (Myrtales)",
  "Nymphaeales"     = "Water-lilies (Nymphaeales)",
  "Ophioglossales"  = "Adder's-tongue (Ophioglossales)",
  "Oxalidales"      = "Wood-sorrels (Oxalidales)",
  "Pinales"         = "Pines (Pinales)",
  "Poales"          = "Grasses (Poales)",
  "Polypodiales"    = "True ferns (Polypodiales)",
  "Ranunculales"    = "Buttercups (Ranunculales)",
  "Rosales"         = "Roses (Rosales)",
  "Salviniales"     = "Floating ferns (Salviniales)",
  "Santalales"      = "Sandalwoods (Santalales)",
  "Sapindales"      = "Maple & citrus (Sapindales)",
  "Saxifragales"    = "Saxifrages (Saxifragales)",
  "Selaginellales"  = "Spikemosses (Selaginellales)",
  "Solanales"       = "Nightshades (Solanales)"
)
...
## Priority: confirmed > new > reported for cumulative table
## NOTE:
## keeping your original logic where confirmed overrides new if present
## ------------------------------------------------------------

taxon_order_status <- dat %>%
  group_by(Order_std, Taxon_std) %>%
  summarise(
...
order_diversity_summary <- taxon_order_status %>%
  count(Order_std, Order_label, knowledge_category, name = "n_taxa") %>%
  group_by(Order_std, Order_label) %>%
  mutate(total_taxa = sum(n_taxa)) %>%
  ungroup()

order_order <- order_diversity_summary %>%
  distinct(Order_std, Order_label, total_taxa) %>%
  arrange(desc(total_taxa), Order_std) %>%
  pull(Order_std)

order_diversity_summary$Order_std <- factor(
  order_diversity_summary$Order_std,
  levels = order_order
)
...
order_label_levels <- order_diversity_summary %>%
  distinct(Order_std, Order_label) %>%
  mutate(Order_std = factor(Order_std, levels = order_order)) %>%
  arrange(Order_std) %>%
  pull(Order_label)
...
cat("\nCumulative knowledge summary by vascular plant order:\n")
print(order_diversity_summary, n = nrow(order_diversity_summary))
...
p_cumulative_knowledge <- ggplot(
  ...
  labs(
    title = "Biodiversity Galiano 2026: cumulative knowledge by order",
...
## ------------------------------------------------------------
## RANDOM EXAMPLE OBSERVATIONS TABLE BY ORDER × CATEGORY
## ------------------------------------------------------------
...
write.csv(
  example_table,
  file.path(OUTPUT_DIR, "Vascular_plants_random_example_observations_by_order_recent_new_records.csv"),
  row.names = FALSE
)
...
## ------------------------------------------------------------
## RANDOM STORY EXAMPLES BY ORDER + STORY TEXT
## ------------------------------------------------------------
...
vascular_plant_story_examples <- example_pool_story %>%
  group_by(Order_std, Order_label, Knowledge_category) %>%
  slice_sample(n = 1) %>%
  ungroup() %>%
  mutate(
...
print(vascular_plant_story_examples, n = nrow(vascular_plant_story_examples))

write.csv(
  vascular_plant_story_examples,
  file.path(OUTPUT_DIR, "Vascular_plants_reports_by_order.csv"),
  row.names = FALSE
)
...
## ------------------------------------------------------------
## BROAD-BRUSH DIVERSITY SUMMARY BY ORDER
## ------------------------------------------------------------
...
write_csv(
  order_summary_out,
  file.path(OUTPUT_DIR, "Vascular_plant_diversity_summary_by_order.csv")
)
...
cat("\n")
cat("============================================================\n")
cat("VASCULAR PLANT DIVERSITY: BROAD-BRUSH SUMMARY BY ORDER\n")
cat("============================================================\n\n")
...
if (overall$recent_total > 0) {
  cat(
    paste0(
      ...
      n_fmt(overall$recent_new), " through newly reported additions to the island flora.\n\n"
    )
  )
} else {
  cat(
...
if (nrow(top_confirmed) > 0) {
  ...
  if (any(top_confirmed$recent_confirmed > 0)) {
    cat(
      paste0(
        "Within the recent window, the leading orders for historical confirmation were ",
        collapse_and(
          paste0(
...
if (nrow(top_recent) > 0) {
  cat(
    paste0(
      "Looking specifically at recent activity, the orders showing the most momentum in ",
      RECENT_YEAR_MIN, "–", RECENT_YEAR_MAX, " were ",
      collapse_and(paste0(top_recent$Order_label, " (", top_recent$recent_total, ")")),
      ". These groups account for much of the recent growth in contemporary knowledge of the island flora.\n\n"
    )
  )
}
...
if (nrow(top_attention) > 0) {
  cat(
    paste0(
      "The orders still requiring the most attention for confirmation of historical reports are ",
      collapse_and(
        paste0(
  ...
  cat(
    paste0(
      "These orders stand out either because they contain the largest absolute backlog of historical reports, ",
...
cat(
  paste0(
    ...
    " indicates that contemporary community observations are continuing to make measurable gains, while also helping reveal which orders still contain the largest historical knowledge gaps.\n\n"
  )
)
