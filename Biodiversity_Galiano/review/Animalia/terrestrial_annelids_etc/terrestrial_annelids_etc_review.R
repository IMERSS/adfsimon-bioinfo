...
baseline <- read.csv("summaries/Galiano_terrestrial_annelids_etc_review_summary_2023-11-04.csv")
...
iNat.obs.summary <- read.csv("../../../parse_iNat_records/outputs/iNat_obs_Terrestrial_annelids_etc.csv")
...
iNat.obs.summary$taxon_variety_name <- NA
...
rotifers <- unmatched.iNat.obs.summary %>% filter(Phylum == 'Rotifera')
annelids <- unmatched.iNat.obs.summary %>% filter(Phylum == 'Annelida')

rotifers$Stats.Code <- 'ROT'
annelids$Stats.Code <- 'WOR'

unmatched.iNat.obs.summary <- rbind(rotifers,annelids)
...
write.csv(review.summary, "outputs/Galiano_terrestrial_terrestrial_annelids_etc_review_summary.csv", row.names = FALSE)
