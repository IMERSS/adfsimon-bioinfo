...
change_log_path <- "outputs/Galiano_terrestrial_annelids_etc_exclusion_change_log.csv"
harvested_rules_path <- "outputs/Galiano_terrestrial_annelids_etc_manual_rules_harvested_this_cycle.csv"
output_path <- "outputs/Galiano_terrestrial_annelids_etc_review_summary.csv"
...
require_cols(
  iNat.obs.summary,
  c("scientific_name", "observed_on", "user_name", "taxon_rank", "genus", "family", "taxon_id", "id", "phylum"),
...
# Drop extraneous fields if present
if ("X" %in% names(iNat.obs.summary)) {
  iNat.obs.summary <- iNat.obs.summary %>% select(-X)
}
