...
summary <- read.csv("resynthesize/Animalia/marine_animals/summaries/Galiano_marine_animals_resynthesized_summary_2023-11-07.csv")

hulq <- read.csv("analyze/Animalia/marine_animals/hulq_analysis/tabular_data/reintegrated-with-hulq.csv")

animals <- read.csv("consolidate_records/Animalia/marine_animals/synthesized/Galiano_marine_animal_records_consolidated-2023-11-07.csv")
...
# Filter animal summary and records to culturally significant taxa

hulq.names <- hulq$Taxon.name

summary <- summary %>% filter(scientificName %in% hulq.names)
...
annelida        <- list(summary = summary %>% filter(phylum == 'Annelida'), records = cultural.animals %>% filter(phylum == 'Annelida'))
brachiopoda     <- list(summary = summary %>% filter(phylum == 'Brachiopoda'), records = cultural.animals %>% filter(phylum == 'Brachiopoda'))
bryozoa         <- list(summary = summary %>% filter(phylum == 'Bryozoa'), records = cultural.animals %>% filter(phylum == 'Bryozoa'))
chaetognatha    <- list(summary = summary %>% filter(phylum == 'Chaetognatha'), records = cultural.animals %>% filter(phylum == 'Chaetognatha'))
cnidaria        <- list(summary = summary %>% filter(phylum == 'Cnidaria'), records = cultural.animals %>% filter(phylum == 'Cnidaria'))
crustacea       <- list(summary = summary %>% filter(subphylum == 'Crustacea'), records = cultural.animals %>% filter(subphylum == 'Crustacea'))
ctenophora      <- list(summary = summary %>% filter(phylum == 'Ctenophora'), records = cultural.animals %>% filter(phylum == 'Ctenophora'))
echinodermata   <- list(summary = summary %>% filter(phylum == 'Echinodermata'), records = cultural.animals %>% filter(phylum == 'Echinodermata'))
entoprocta      <- list(summary = summary %>% filter(phylum == 'Entoprocta'), records = cultural.animals %>% filter(phylum == 'Entoprocta'))
mammalia        <- list(summary = summary %>% filter(class == 'Mammalia'), records = cultural.animals %>% filter(class == 'Mammalia'))
mollusca        <- list(summary = summary %>% filter(phylum == 'Mollusca'), records = cultural.animals %>% filter(phylum == 'Mollusca'))
nemertea        <- list(summary = summary %>% filter(phylum == 'Nemertea'), records = cultural.animals %>% filter(phylum == 'Nemertea'))
phoronida       <- list(summary = summary %>% filter(phylum == 'Phoronida'), records = cultural.animals %>% filter(phylum == 'Phoronida'))
platyhelminthes <- list(summary = summary %>% filter(phylum == 'Platyhelminthes'), records = cultural.animals %>% filter(phylum == 'Platyhelminthes'))
porifera        <- list(summary = summary %>% filter(phylum == 'Porifera'), records = cultural.animals %>% filter(phylum == 'Porifera'))
sipuncula       <- list(summary = summary %>% filter(phylum == 'Sipuncula'), records = cultural.animals %>% filter(phylum == 'Sipuncula'))
tunicata        <- list(summary = summary %>% filter(subphylum == 'Tunicata'), records = cultural.animals %>% filter(subphylum == 'Tunicata'))
fishes          <- list(summary = summary %>% filter(class == 'Actinopterygii'| class == 'Elasmobranchii'), records = cultural.animals %>% filter(class == 'Actinopterygii'| class == 'Elasmobranchii'))
