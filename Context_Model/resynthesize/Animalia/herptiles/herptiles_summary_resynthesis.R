...
baseline <- read.csv("../../../review/Animalia/herptiles/summaries/Context_model_herptiles_review_summary_2023-12-25.csv")

# Frame baseline using new school template

# Define fields for new school summary

Fields <- c('scientificName', 'scientificNameAuthorship','subtaxonAuthorship','commonName','kingdom','phylum', 
...
# Create new school summary template 

data.frame <- as.data.frame(matrix(ncol = length(Fields), nrow = nrow(baseline)))
names(data.frame) <- Fields

data.frame[names(baseline)] <- baseline

baseline <- select(data.frame, c(1:length(Fields)))

# Read catalog of consolidated occurrence records

records <- read.csv("../../../consolidate_records/Animalia/herptiles/synthesized/Context_model_herptile_records_consolidated_2025-08-16.csv")
...
# Are there taxa uniquely represented by dateless records?
...
# past.records <- past.records %>% filter(!str_detect(datasetName, "UNESCO Nomination")) # Right now, now UNSEC Nomination records to speak of
...
summary$statsCode <- baseline$statsCode[match(unlist(summary$scientificName), baseline$scientificName)]

# Order and output summary

summary <- summary[order(summary$scientificName),] 

write.csv(summary, "outputs/Context_model_herptile_summary_resynthesized.csv", row.names = FALSE, na = '')
